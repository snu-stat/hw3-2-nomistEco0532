...
df_pd["MaritalStatus"].value_counts(drop_nulls=False)
smoke_pl = (
...
def stepwise_aic(data, fixed_terms, candidate_terms, start_terms,
    ...
    best_aic = fit_aic(fixed_terms + included, data)
        ...
        if direction in ("forward", "both"):
            for t in candidate_terms:
                if t not in included:
                    trial = included + [t]
                    moves.append((fit_aic(fixed_terms + trial, data), "add", t, trial))
        if direction in ("backward", "both"):
            for t in included:
                trial = [x for x in included if x != t]
                moves.append((fit_aic(fixed_terms + trial, data), "drop", t, trial))
        if not moves:
            break
...
TeamRuns = (
    ...
    .with_columns(
        ...
        (pl.col("RS")/(pl.col("RA"))).alia("run_ratio")
    )
    .select(["yearID", "teamID", "lgID", "WPct", "run_ratio", "G", "RS", "RA"])
)
```
...
```{python}
X = TeamRuns.select(pl.col("run_ratio").log()).to_numpy()
y = TeamRuns.select("WPct").to_numpy().flatten()
weights = TeamRuns.select("G").to_numpy().flatten()
...
```{python}
X = np.column_stack([
    TeamRuns.select(pl.col("RS").log()).to_numpy().flatten(),
    TeamRuns.select(pl.col("RA").log()).to_numpy().flatten()
])

y       = TeamRuns.select("WPct").to_numpy().flatten()
weights = TeamRuns.select("G").to_numpy().flatten()

# --- Fit GLM (no intercept) ---
fit3 = sm.GLM(y, X,
              family=sm.families.Binomial(),
              var_weights=weights).fit()

print(fit3.summary(yname="WPct", xname=["log(RS)", "log(RA)"]))
```
...
# --- Fit GLM ---
y       = TeamRuns2["WPct"].to_numpy()
weights = TeamRuns2["G"].to_numpy()

X_cols = [c for c in TeamRuns2.columns if c not in ["WPct", "W", "G"]]
X = sm.add_constant(TeamRuns2.select(X_cols).to_numpy())

fit_full = sm.GLM(y, X,
                  family=sm.families.Binomial(),
                  var_weights=weights).fit()

print(fit_full.summary(yname="WPct", xname=["intercept"] + X_cols))
```
...
```{python}
X = TeamRuns2.select(pl.col("run_ratio").log()).to_numpy()
y = TeamRuns2.select("W").to_numpy().flatten()
offset = TeamRuns2.select(pl.col("G").log()).to_numpy().flatten()

fit3 = sm.GLM(y, X,
              family=sm.families.Poisson(),
              offset=offset).fit()

print(fit3.summary(yname="W", xname=["log(run_ratio)"]))
print(fit3.conf_int())
```
...
print(fit_nb.summary(yname="W", xname=["log(run_ratio)"]))
print(fit_nb.conf_int())
```
